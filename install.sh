...
if [[ $EUID -ne 0 ]]; then
    ...
    wget ftp://123.31.12.230/9hits-linux-x64.tar.bz2
...
cat > $settings <<EOFSS
    {"hiddenColumns":[],"token":"$token","browser":"hide","popups":"$pupups","adult":"$adultpages","autoStart":"yes","caching":"2000"}
EOFSS
...
$cronvar
30 7 * * * /usr/local/tmpfs/9Hits/reboot.sh
$cronvar
58 18 * * * /root/9hits.sh
$lookup
EOFSS
